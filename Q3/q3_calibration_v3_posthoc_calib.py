# -*- coding: utf-8 -*-
"""
Q3 校准模型 FINAL v3（RF + O3残差校正 + 回退策略 + 2019异常日剔除 + 更稳健的 Post-hoc 校正）

v3 相比 v2 的改动：
1) Post-hoc 不再用“整个训练集”拟合 a,b，而是用训练末段的“时间校准集”拟合，减少分布漂移导致的过校正；
2) Post-hoc 只有在校准集上能显著降低 RMSE（默认>=0.5% 相对改善）才启用；
3) 默认对 SO2/O3 采用更严格阈值（>=1.0%），避免低信噪变量被过校正；
4) 仍保留 fallback：若测试集校准后 RMSE 变差，则回退到 micro raw（保证不变差）。

运行：
python q3_calibration_v3_posthoc_calib.py --clip_nonneg
如需关闭 posthoc：加 --disable_posthoc
"""
...
def fit_posthoc_linear(y_true, y_pred, min_points=50):
    """
...
def main():
    ...
    parser.add_argument("--posthoc_cal_ratio", type=float, default=0.2,
                        help="Use last X portion of TRAIN as time-calibration set for posthoc.")
    ...
    for target in targets:
        ...
        y_train = (y_station_train - y_base_train) if use_residual else y_station_train
        ...
        best_name, best_model = ("Ridge+Spline", model_linear) if cv_rmse_1 <= cv_rmse_2 else ("RF", model_tree)
        best_model.fit(X_train, y_train)

        # test pred (delivered space)
        pred_test = best_model.predict(X_test).astype(float)
        y_pred = (y_base_test + pred_test) if use_residual else pred_test
        if args.clip_nonneg:
            y_pred = np.clip(y_pred, 0, None)

        # ===== robust posthoc: time-calibration split inside TRAIN =====
        posthoc_used = False
        posthoc_a, posthoc_b = 1.0, 0.0
        posthoc_gain = 0.0

        if not args.disable_posthoc and len(train_df) >= 200:
            # split train into fit/cal by time (use last portion as cal)
            train_sorted = train_df.sort_values("dt").reset_index(drop=True)
            n_tr = len(train_sorted)
            n_cal = max(50, int(round(n_tr * args.posthoc_cal_ratio)))
            n_fit = max(50, n_tr - n_cal)

            fit_df = train_sorted.iloc[:n_fit].copy()
            cal_df = train_sorted.iloc[n_fit:].copy()

            X_fit = fit_df[feats]
            y_base_fit = fit_df[mc_col].values.astype(float)
            y_true_fit = fit_df[st_col].values.astype(float)

            X_cal = cal_df[feats]
            y_base_cal = cal_df[mc_col].values.astype(float)
            y_true_cal = cal_df[st_col].values.astype(float)

            y_fit_target = (y_true_fit - y_base_fit) if use_residual else y_true_fit

            # re-fit a temporary model on fit_df (same algorithm) for stable posthoc
            tmp_model = best_model
            tmp_model.fit(X_fit, y_fit_target)

            cal_raw = tmp_model.predict(X_cal).astype(float)
            y_cal_pred = (y_base_cal + cal_raw) if use_residual else cal_raw

            rmse_before = rmse(y_true_cal, y_cal_pred)

            a, b, ok = fit_posthoc_linear(y_true_cal, y_cal_pred, min_points=50)
            if ok:
                y_cal_post = a * y_cal_pred + b
                if args.clip_nonneg:
                    y_cal_post = np.clip(y_cal_post, 0, None)
                rmse_after = rmse(y_true_cal, y_cal_post)

                # threshold: stricter for SO2/O3
                thr = 0.01 if target in ["SO2", "O3"] else 0.005
                if rmse_before > 1e-9 and (rmse_before - rmse_after) / rmse_before >= thr:
                    posthoc_used = True
                    posthoc_a, posthoc_b = float(a), float(b)
                    posthoc_gain = float(rmse_before - rmse_after)
        ...
        joblib.dump({
            ...
            "posthoc_a": posthoc_a,
            "posthoc_b": posthoc_b,
            "posthoc_gain_calset_rmse": posthoc_gain,
            "posthoc_cal_ratio": args.posthoc_cal_ratio
        }, model_path)
        ...
        if used_fallback:
            ...
            full_raw = best_model.predict(aligned[feats]).astype(float)
            full_y = (aligned[mc_col].astype(float).to_numpy() + full_raw) if use_residual else full_raw
            if posthoc_used:
        ...
        card_text = (
            ...
            f"Posthoc(cal-set) gain: {posthoc_gain:.3f}\n\n"
            f"Before:\n  RMSE={base_m['RMSE']:.2f}, MAE={base_m['MAE']:.2f}\n"
            f"  R2={base_m['R2']:.3f}, CCC={base_m['CCC']:.3f}\n\n"
            f"After:\n  RMSE={calib_m['RMSE']:.2f}, MAE={calib_m['MAE']:.2f}\n"
